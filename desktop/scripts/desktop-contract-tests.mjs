...
test('desktop graphics and window activation keep the P0 fast path wired', () => {
  ...
  assert.match(main, /process\.env\.METIS_WINDOWS_IDENTITY_VERIFY === '1'/);
...
test('NEW-68 provider usage and model catalog stays wired', () => {
  ...
  assert.match(settings, /读取模型列表/);
  assert.match(settings, /额度 \/ Usage/);
  assert.match(settings, /provider-model-summary/);
...
test('NEW-72 settings model and web preview polish stays wired', () => {
  ...
  assert.match(settings, /endpointSettings/);
  assert.match(settings, /remoteOnly:\s*true/);
  assert.match(settings, /provider-model-summary/);
  assert.match(rightRail, /previewLoad/);
  assert.match(rightRail, /previewSetBounds/);
