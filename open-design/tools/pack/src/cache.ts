...
export async function renameDirectoryWithRetry(
  ...
  const maxRetries = Math.max(0, options.maxRetries ?? (process.platform === "win32" ? 30 : 0));
  ...
  for (let attempt = 0; ; attempt += 1) {
    try {
      ...
      if (attempt >= maxRetries || !["EPERM", "EBUSY", "EACCES"].includes(code)) throw error;
      await wait(Math.min(2000, 100 * (attempt + 1)));
    }
  }
}
    ...
    const manifest = await withDirectoryLock(join(this.root, "locks"), "global", async () => {
      ...
      const nextManifest = manifest ?? await (async () => {
        ...
        try {
          ...
          await renameDirectoryWithRetry(stagingPath, entryPath);
