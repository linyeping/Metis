...
import { foregroundTargetsAreIdle } from "./foreground-lifecycle.js";
...
async function spawnSidecarRuntime(request: {
  ...
  const spawned = await spawnBackgroundProcess({
    ...
    // A detached Node process opens a visible Windows Terminal window on
    // systems that use it as the default console host. Metis owns and stops
    // these sidecars, so its managed runtime keeps them attached and hidden.
    detached: process.env.METIS_MANAGED_DESIGN_RUNTIME !== "1",
    env: {
      ...process.env,
      ...env,
      ...request.env,
...
async function runForeground(config: ToolDevConfig, appName: string | undefined, options: CliOptions) {
  ...
  await new Promise<void>((resolveDone) => {
    let lifecycleWatch: NodeJS.Timeout | null = null;
    const shutdown = () => {
      if (shuttingDown) return;
      shuttingDown = true;
      clearInterval(keepAlive);
      if (lifecycleWatch != null) clearInterval(lifecycleWatch);
      process.stderr.write("\nStopping Metis Design dev server...\n");
    ...
    lifecycleWatch = setInterval(() => {
      if (shuttingDown || checkingLifecycle) return;
      checkingLifecycle = true;
      void Promise.all(targets.map((target) => inspectAppStatus(config, target)))
        .then((statuses) => {
          if (foregroundTargetsAreIdle(statuses)) shutdown();
        })
