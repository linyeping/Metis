...
export const PLUGIN_SHARE_ACTION_LABELS: Record<PluginShareAction, string> = {
  ...
  'contribute-open-design': 'Contribute to Metis Design',
};
...
export function renderPluginSharePrompt({ action, sourcePlugin, stagedPath }: PluginSharePromptInput): string {
  ...
  if (action === 'publish-github') {
    return [
      `Publish the local Metis Design plugin "${title}" as a new public GitHub repository.`,
      '',
      `The plugin source files have been copied into this project at \`${stagedPath}\`.`,
      'Use the local daemon share endpoint so the publish flow runs through Metis Design\'s validated GitHub path:',
      '',
      '```bash',
  ...
  return [
    `Open a pull request to add the local Metis Design plugin "${title}" to the Metis Design repository.`,
    '',
    `The plugin source files have been copied into this project at \`${stagedPath}\`.`,
    'Use the local daemon share endpoint so the contribution flow runs through Metis Design\'s validated GitHub path:',
    '',
    '```bash',
...
export function upsertSkillPluginCandidateAssistantMessage(
  ...
  upsertMessage(db, run.conversationId, {
    id: messageId,
    role: 'assistant',
    content: `Metis Design found reusable skill material that can become a plugin: ${candidate.title}`,
    events: [{
