...
import { createMetisHttpTransport } from './runtimes/metis-http.js';
import {
...
const METIS_PERMISSION_CHOICES = new Set([
  'once',
  'always_allow',
  'temporary_root',
  'writable_root',
...
export function resolveManagedMetisBackendOrigin(env: NodeJS.ProcessEnv = process.env): string | null {
  if (env.METIS_MANAGED_DESIGN_RUNTIME !== '1' || !env.METIS_DESIGN_TOKEN) return null;
  try {
    const parsed = new URL(String(env.METIS_BACKEND_URL || ''));
...
export function createAgentRuntimeToolPrompt(
  ...
  return [
    ...
    '- `OD_BIN` is the absolute path to the Metis Design CLI script. On POSIX shells run wrappers with `"$OD_NODE_BIN" "$OD_BIN" tools ...`; do not call bare `od`, which may resolve to the system octal-dump command on Unix-like systems.',
...
export async function startServer({
  ...
  app.post('/api/metis/permissions/:requestId/answer', requireLocalDaemonRequest, async (req, res) => {
    const origin = resolveManagedMetisBackendOrigin();
    const requestId = String(req.params.requestId || '').trim();
    const choice = String(req.body?.choice || '').trim();
    if (!origin) return res.status(404).json({ error: 'Metis managed Design runtime is unavailable.' });
    if (!/^[A-Za-z0-9._:-]{1,128}$/.test(requestId)) {
      return res.status(400).json({ error: 'Invalid permission request id.' });
    }
    if (!METIS_PERMISSION_CHOICES.has(choice)) {
      return res.status(400).json({ error: 'Unsupported permission choice.' });
    }
    try {
      const response = await fetch(`${origin}/permissions/requests/${encodeURIComponent(requestId)}/answer`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'X-Metis-Design-Token': String(process.env.METIS_DESIGN_TOKEN || ''),
        },
      ...
      if (!response.ok) {
        return res.status(response.status).json({ error: String(payload?.error || `Metis permission request failed (${response.status}).`) });
      }
      return res.json({ ok: true, approved: Boolean(payload?.approved), request: payload?.request || null });
    } catch (error) {
  ...
  const startChatRun = async (chatBody, run) => {
    ...
    const agentLaunch = def.transport === 'metis-http'
      ? {
          selectedPath: 'metis-native',
          launchPath: 'metis-native',
          launchKind: 'selected',
          childPathPrepend: [],
          diagnostic: null,
        }
    ...
    // Metis Design relays them to the agent so the model can call those tools.
    ...
    try {
      ...
      if (def.transport === 'metis-http') {
        child = createMetisHttpTransport({
          backendUrl: String(env.METIS_BACKEND_URL || ''),
          token: String(env.METIS_DESIGN_TOKEN || ''),
          designRoot: String(env.METIS_DESIGN_ROOT || ''),
          stateRoot: String(env.OD_DATA_DIR || RUNTIME_DATA_DIR),
    ...
    } else if (def.streamFormat === 'metis-http') {
      trackingSubstantiveOutput = true;
      child.on('agent', sendAgentEvent);
  ...
  orbitService.setRunHandler(async ({
    ...
    design.runs.start(run, () => startChatRun({
      ...
      systemPrompt: [
        ...
        systemPrompt,
        'You are Orbit, an autonomous activity-summary agent inside Metis Design.',
