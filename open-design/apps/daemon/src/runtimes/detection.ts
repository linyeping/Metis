...
async function probe(
  ...
  if (def.transport === 'metis-http') {
    const backendUrl = String(process.env.METIS_BACKEND_URL || '').trim();
    const token = String(process.env.METIS_DESIGN_TOKEN || '').trim();
    const available = /^https?:\/\/(?:127\.0\.0\.1|localhost|\[::1\])(?::\d+)?$/i.test(backendUrl) && Boolean(token);
    return {
      ...stripFns(def),
      models: def.fallbackModels,
      modelsSource: 'fallback',
      available,
      ...(available ? { path: 'metis-native' } : {}),
      version: available ? 'native' : null,
    };
  }
