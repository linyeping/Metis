      ...
      typeof data.detail === 'string'
        ...
        : typeof data.model === 'string'
          ...
          : typeof data.ttftMs === 'number'
            ...
            : undefined;
    const permission = isRecord(data.permission) ? data.permission : null;
    return {
      kind: 'status',
      label: data.label,
      ...(detail ? { detail } : {}),
      ...(permission ? {
        permission: permission as unknown as NonNullable<Extract<PersistedAgentEvent, { kind: 'status' }>['permission']>,
      } : {}),
    };
  }
