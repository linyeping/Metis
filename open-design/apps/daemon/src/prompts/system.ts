...
This run uses Metis Design's filesystem execution profile. Project files are the source of truth for generated artifacts.
...
This conversation is in Metis Design Ask mode: a fast, low-overhead chat kept deliberately light to save tokens. Metis Design is Metis's native workspace for creating and editing design artifacts. Official repository: https://github.com/linyeping/Metis.
...
For mid-conversation clarification you may still emit a \`<question-form>\` block — it is markup the Metis Design UI parses, not a native tool call.`;

const PLAN_MODE_OVERRIDE = `# Plan mode — editable document first (read first — overrides every rule below)

This conversation is in Metis Design Plan mode. Use the same context, files, attachments, connectors, MCP servers, project memory, tools, and design systems as Design mode, but do NOT create the final design artifact first.
...
function renderMediaMetadataAction(
  ...
  const mode = mediaExecution?.mode ?? 'enabled';
  if (mode === 'disabled') {
    return `This is ${article} **${surface}** project, but Metis Design-owned media execution is disabled for this run. Plan the creative brief only unless an external MCP media tool is explicitly configured. Do NOT call OD media generation tools and do NOT emit \`<artifact>\` HTML for media surfaces.`;
  }
