...
describe('buildPublishLink', () => {
  ...
  it('builds an Open Design registry submission URL', () => {
    ...
    // operational launch step happens, submissions land in `linyeping/Metis`
    // (open-design/plugins/community/<plugin-name>/), keeping contribution in
    // the Metis monorepo.
    const link = buildPublishLink({ catalog: 'open-design', meta: META });
    expect(link.catalogLabel).toBe('linyeping/Metis');
    expect(link.url).toMatch(/^https:\/\/github\.com\/linyeping\/Metis\/issues\/new\?/);
    expect(link.prBody).toContain('open-design/plugins/community/<plugin-name>/open-design.json');
    expect(link.prBody).toContain('open-design/plugins/registry/community/open-design-marketplace.json');
  });
