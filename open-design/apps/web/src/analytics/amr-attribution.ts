...
// Builds the AMR handoff URL with Metis Design attribution params. When
// `deviceId` is provided it is added as `od_device_id`, so AMR can link the
// landing/registration directly back to this Metis Design install instead of
// only through the one-shot entry id. The caller passes it ONLY when the user
// has consented to metrics: AMR is Metis Design's official model service, so
...
async function mirrorAmrEntryToAmrAnalytics(
  ...
  try {
    ...
    // AMR analytics mirroring must never block the primary Metis Design action.
  }
}
