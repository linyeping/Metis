...
// `contribute` opens a draft PR against the Metis community repository
...
function buildContributePrompt(folderPath: string): string {
  return [
    'Open a draft Pull Request that adds this generated plugin to the Metis Design community catalog at `linyeping/Metis`.',
    ...
    `Manifest: \`${folderPath}/open-design.json\``,
    '',
    'Run this deterministic Metis Design CLI workflow from the current project workspace:',
    '',
...
// Metis Design issue URL and belongs to the "Metis Design PR" button. Before
...
function buildPublishPrompt(folderPath: string): string {
  return [
    ...
    `Manifest: \`${folderPath}/open-design.json\``,
    '',
    'This is the **repository publish** action, NOT the registry-submission action — do NOT route through `od plugin publish --to open-design`. That command emits a Metis Design issue URL and belongs to the "Metis Design PR" button.',
    '',
    'Run this deterministic Metis Design CLI workflow from the current project workspace:',
    '',
