...
type ProjectChatSendMeta = ChatSendMeta & {
  ...
  queueDrain?: boolean;
  /** The Metis Design Cloud balance gate already ran for this exact send at
...
function historyWithWorkspaceContext(
  ...
  const block = [
    ...
    'Metis Design selected or inferred these workspace contexts for this turn. Treat absolute paths as reference context unless the user explicitly asks to edit them.',
...
/** Set by the home create flow when its submit already ran the Metis Design
...
export function ProjectView({
  ...
  const handleSend = useCallback(
    async (
      ...
      // Metis Design Cloud pre-run balance gate: a definitively insufficient
  ...
  // "Share to Metis Design" — kicks off the bundled `od-share-to-community`
...
function pluginWorkflowTitle(action: PluginFolderAgentAction): string {
  return action === 'publish' ? 'Publish repo' : 'Metis Design PR';
}
...
function pluginWorkflowPlannedSteps(action: PluginFolderAgentAction): string[] {
  ...
  return [
    'Ensure the Metis Design fork exists',
...
const DESIGN_RESULT_DELIVERY_FAILED_DETAIL =
  'The design result was generated, but Metis Design could not save it to the project.';
