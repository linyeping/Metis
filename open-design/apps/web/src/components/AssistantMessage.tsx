...
function SkillPluginCandidateCard({
  ...
  async function share(action: "contribute-open-design") {
    ...
    try {
      ...
      setNotice({
        message: `Metis Design contribution task started for ${data?.path ?? "the draft"}.`,
      });
    } catch (err) {
      setNotice({ message: err instanceof Error ? err.message : String(err) });
    } finally {
...
interface Props {
  ...
  // Click handler for the post-completion "Share to Metis Design" submission
      ...
      <div className="assistant-flow">
        {blocks.map((b, i) => {
          ...
          if (b.kind === "status") {
            ...
            if (b.label === "waiting_permission" && b.permission) {
              return <PermissionCard key={b.permission.requestId || i} permission={b.permission} />;
            }
...
function hasPluginFinalActionHint(content: string): boolean {
  return /\b(Add to My plugins|Metis Design PR|Publish repo|plugin publish|ready to publish|ready to add)\b/i.test(
    content,
  );
}
...
function PermissionCard({ permission }: { permission: AgentPermissionRequest }) {
  const t = useT();
  const [submitting, setSubmitting] = useState("");
  const [localOutcome, setLocalOutcome] = useState<{ approved: boolean; choice: string } | null>(null);
  const [error, setError] = useState("");
  const resolved = localOutcome !== null || !["requested", "displayed", "waiting"].includes(permission.status);
  const approved = localOutcome?.approved ?? permission.approved;
  const selectedChoice = localOutcome?.choice || permission.selectedChoice || "";
  const selectedLabel = permission.choices.find((choice) => choice.value === selectedChoice)?.label || selectedChoice;
  const choices = permission.choices.filter((choice) => !choice.requiresRootPicker);

  const answer = async (choice: AgentPermissionRequest["choices"][number]) => {
    if (!permission.requestId || submitting || resolved) return;
    setSubmitting(choice.value);
    setError("");
    try {
      const response = await fetch(`/api/metis/permissions/${encodeURIComponent(permission.requestId)}/answer`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
      ...
      setLocalOutcome({ approved: Boolean(payload?.approved), choice: choice.value });
    } catch (cause) {
  ...
  return (
    <section className="permission-card" data-risk={permission.riskLevel?.toLowerCase() || "unknown"}>
      <div className="permission-card-heading">
        <Icon name="lock" size={15} />
        <strong>{t("plugins.availableDetails.permissions")}</strong>
        <code>{permission.toolName}</code>
      </div>
      {permission.explanation ? <p>{permission.explanation}</p> : null}
      {permission.reasoning ? <p className="permission-card-muted">{permission.reasoning}</p> : null}
      {permission.risk ? <p className="permission-card-risk">{permission.risk}</p> : null}
      {resolved ? (
        <div className="permission-card-outcome" data-approved={approved === true ? "true" : "false"}>
          <Icon name={approved === true ? "check" : "alert-triangle"} size={14} />
          <span>{selectedLabel || (approved === true ? "Allowed" : t("automations.reject"))}</span>
        </div>
      ) : (
        <div className="permission-card-actions">
          {choices.map((choice) => (
            <button
              key={choice.value}
              type="button"
              className={choice.approved ? "permission-action-allow" : "permission-action-deny"}
              disabled={Boolean(submitting)}
              title={choice.description}
              onClick={() => void answer(choice)}
            >
              {submitting === choice.value ? <Icon name="spinner" size={13} /> : null}
              <span>{choice.label}</span>
            </button>
          ))}
        </div>
      )}
      {error ? <div className="permission-card-error">{error}</div> : null}
    </section>
  );
}
...
type Block =
  ...
  | { kind: "status"; label: string; detail?: string | undefined; permission?: AgentPermissionRequest };
  ...
  for (const ev of events) {
    ...
    if (ev.kind === "status") {
      ...
      if (last && last.kind === "status" && last.label === ev.label) {
        ...
        last.permission = ev.permission;
        continue;
      }
      out.push({ kind: "status", label: ev.label, detail: ev.detail, permission: ev.permission });
      continue;
    }
  }
  return out;
