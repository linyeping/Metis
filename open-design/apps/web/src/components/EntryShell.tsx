...
type EntryCreateProjectInput = Omit<CreateInput, 'metadata'> & {
  ...
  /** The home submit already ran the Metis Design Cloud balance gate; the
...
interface Props {
  ...
  // replaced by the profile and brand-extraction steps, so EntryShell does
...
export function EntryShell({
  ...
  async function handlePluginLoopSubmit(payload: PluginLoopSubmit) {
    // Metis Design Cloud pre-run balance gate: hard blocks (empty wallet or
...
function OnboardingView({
  ...
  // ref before React's state→ref sync effect runs — `emitAboutYouSubmit` and
  // the Memory note never see a stale field when the user changes an answer
  ...
  // Metis Design Cloud sign-in in one gesture. Mirrors the old AMR card's
  ...
  // `profileRef` captures the user's
  ...
  // Connect step, default face: a minimal, centered Metis Design Cloud sign-in
    ...
    return (
      ...
      >
        ...
        <footer className="onboarding-cloud__footer">
          © {new Date().getFullYear()} Metis Design · {t('settings.onboardingCloudRights')}
        </footer>
      </section>
    );
  }
