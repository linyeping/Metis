...
// `linyeping/Metis`; `publish` drives `gh repo create / push` against the
...
describe('buildPluginFolderAgentActionPrompt', () => {
  ...
  describe('publish (repo-publish flow)', () => {
    ...
    it('bans the registry-submission CLI explicitly', () => {
      ...
      expect(prompt).toMatch(
        /registry[- ]submission|registry-submission flow|Metis Design PR/i,
      );
    });
  ...
  describe('contribute (PR-based flow)', () => {
    ...
    it('delegates Metis Design PR creation to the deterministic plugin CLI helper', () => {
      expect(prompt).toContain('linyeping/Metis');
