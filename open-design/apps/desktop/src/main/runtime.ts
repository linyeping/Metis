...
// sees the web's own "Loading Metis Design…" shell flash between the splash and
...
function buildCrashReportUrl(ctx: RendererCrashScreenContext): string {
  ...
  const body = [
    "**What happened**",
    "The Metis Design desktop window crashed several times in a row and showed the recovery screen.",
    "",
...
function buildCrashMailtoUrl(ctx: RendererCrashScreenContext): string {
  const subject = `Metis Design keeps crashing (renderer ${ctx.reason})`;
  const body = [
    "The Metis Design desktop app crashed several times in a row on my device.",
    "",
...
<html>
  ...
  <body>
    <div class="panel">
      <p class="title">Metis Design keeps closing on this device</p>
      ...
      <p class="hint">If this keeps happening, quitting and reinstalling Metis Design usually resolves it.</p>
    </div>
    <script>
      (function () {
...
const SPLASH_STAGE_LABELS: Record<SplashBootStage, string> = {
  starting: "Starting Metis Design",
...
export async function createDesktopRuntime(options: DesktopRuntimeOptions): Promise<DesktopRuntime> {
  ...
  const windowTitle = options.windowTitle ?? "Metis Design";
  const window = new BrowserWindow({
    ...
    // web's own "Loading Metis Design…" shell.
    show: false,
    title: windowTitle,
    autoHideMenuBar: true,
    ...MAC_WINDOW_CHROME,
  ...
  const requireMainWindowSender = (event: Electron.IpcMainInvokeEvent): void => {
    if (event.sender !== window.webContents) {
      throw new Error("host IPC is only available to the main Metis Design window");
    }
  };
  ...
  // reveal the web's own dark "Loading Metis Design…" shell, and (b) the splash
