export const OPEN_DESIGN_GITHUB_REPO_URL = 'https://github.com/linyeping/Metis';
...
export function buildSocialSharePayload(input: SocialShareRequest): SocialShareResponse {
  ...
  const fallbackTitle = kind === 'project-html' ? 'Metis Design project' : 'Metis Design';
  const title = cleanText(input.title, fallbackTitle);
  const fallbackText = kind === 'project-html'
    ? `Built with Metis Design: ${title}. Metis Design repo: ${OPEN_DESIGN_GITHUB_REPO_URL}`
    : 'Metis Design is an open-source workspace for creating, editing, deploying, and handing off design artifacts.';
