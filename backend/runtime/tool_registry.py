...
_TOOL_USAGE_HINTS: Dict[str, str] = {
    ...
    "metis_vm_direct_assets_prepare": "Prepare Metis-owned direct VM assets, including the formatted sessiondata-template.vhdx used to clone per-session persistent disks.",
    "metis_vm_direct_runner_prepare": "Prepare the Metis direct VM runner contract: host HCS runner, guest metisd JSONL daemon, artifact sync, diagnostics, lifecycle schema, and manifest. Production use remains readiness-gated.",
    ...
    "metis_vm_hcs_starter_prepare": "Prepare the Windows HCS ComputeSystem starter assets: compute-system JSON, C# HCS API bridge, PowerShell starter, and manifest. It does not start the VM.",
    ...
    "metis_vm_guest_handshake_verify": "Prove guest metisd answers runtime.hello through the production HCS service; HCS success records a bundle-bound readiness receipt.",
    ...
    "metis_vm_rootfs_boot_verify": "Dry-run or explicitly execute HCS boot attempts across rootfs cmdline candidates. Only the production handshake and persistence selftest can mark runner_ready.",
